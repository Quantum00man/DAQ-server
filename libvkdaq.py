...
library_name = "libvkdaq.dll" if os.name == "nt" else "libvkdaq.so"
default_home = os.path.dirname(__file__) if os.name == "nt" else "/opt/vkdaq/lib"
vkdaq_home = os.getenv("VKDAQ_HOME", default_home)
dll_path = os.path.abspath(
    os.getenv("VKDAQ_LIBRARY", os.path.join(vkdaq_home, library_name))
)
if not os.path.isfile(dll_path):
    raise OSError(f"Vkinging DAQ library not found: {dll_path}")

# Python 3.8+ requires dependent Windows DLL directories to be registered.
_dll_directory_handle = None
if os.name == "nt" and hasattr(os, "add_dll_directory"):
    _dll_directory_handle = os.add_dll_directory(os.path.dirname(dll_path))

vkdaq = ctypes.CDLL(dll_path)
VKDAQ_LIBRARY_PATH = dll_path
...
VkDaqGetTaskAttribute.restype =c_int
...
VkDaqSetTaskAttribute.restype =c_int
...
VkDaqGetTaskData.restype =c_int
...
VkDaqGetChannels.restype =c_int
...
VkDaqGetDeviceAttribute.restype =c_int
...
VkDaqSetDeviceAttribute.restype =c_int
...
VkDaqGetChannelAttribute.restype =c_int
...
VkDaqSetChannelAttribute.restype =c_int
...
VkDaqCreateAIVoltageChan.restype =c_int
...
VkDaqCreateAICurrentChan.restype =c_int
...
VkDaqCreateAIAccelChan.restype =c_int
...
VkDaqCreateDIChan.restype =c_int
...
VkDaqCreateMIChan.restype =c_int
...
VkDaqCreateAOVoltageChan.restype =c_int
...
VkDaqCfgSampClkTiming.restype =c_int
...
VkDaqCfgDigEdgeRefTrig.restype =c_int
...
VkDaqCfgAnlgEdgeRefTrig.restype =c_int
...
VkDaqAssistantDisplay.restype =c_int
...
VkDaqGetLastErrorInfo.restype=c_char_p
